...
import pickle
# --------------------------------------------------------------
def load_my_embedding(path,terms,embed):
	idx = int(path[-1])
	X = embed[idx]
	doc_id = [str(i) for i in range(embed[idx].shape[0])]
	return X,terms,doc_id
...
def main():
	...
	for matrix_filepath in args:
		...
		(X,terms,doc_ids) = load_my_embedding(matrix_filepath,terms,embed)#text.util.load_corpus( matrix_filepath )
